...
interface OfficerLoginPageProps {
  ...
  onOfficerLoginSuccess?: (officer: { username: string }) => void;
}
...
export const OfficerLoginPage: React.FC<OfficerLoginPageProps> = ({
  ...
  const handleOfficerLogin = (e: React.FormEvent) => {
    ...
    setAuthError(null);
    setSuccessToast(`Nodal Officer session authenticated for ${username}. Accessing Redressal Management Portal...`);

    setTimeout(() => {
      if (onOfficerLoginSuccess) {
        onOfficerLoginSuccess({ username });
      } else {
        onNavigate('landing');
      }
    }, 1500);
        ...
        <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-white/90 p-1.5 rounded-lg shadow-md shrink-0">
              <NationalEmblem className="w-10 h-14" showMotto={false} />
            </div>
            <div>
