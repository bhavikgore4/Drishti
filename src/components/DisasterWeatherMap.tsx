...
export const DisasterWeatherMap: React.FC<DisasterWeatherMapProps> = ({
  ...
  const [inspectedHotspot, setInspectedHotspot] = useState<RiskHotspot | null>(NAGPUR_77_HOTSPOTS[0]);
  ...
  const processedHotspots = useMemo(() => {
    return NAGPUR_77_HOTSPOTS.map((h) => {
  ...
  const handleZoomToFitAll = () => {
    ...
    const latLngs: L.LatLngExpression[] = NAGPUR_77_HOTSPOTS.map((h) => [h.lat, h.lng]);
            ...
            <div className="p-3 border-b border-slate-200 space-y-2 bg-slate-50/80">
              <div className="flex items-center justify-between text-[11px]">
                <span className="font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  <span>Risk Filter ({filteredHotspots.length}/77)</span>
                </span>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
